...
import { Utensils, ShoppingCart, Loader2, Search, X, ChefHat, Check, Camera, BookOpen } from 'lucide-react';
    ...
    <div className="h-full flex flex-col bg-white overflow-y-auto">
      <div className="bg-orange-50 border-b border-orange-100 p-6 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <form onSubmit={(e) => { e.preventDefault(); loadPlan(customQuery, `Nutri Heal: ${customQuery}`); }} className="flex gap-2 w-full md:w-auto flex-1 max-w-md">
            <input type="text" placeholder="Ailment..." value={customQuery} onChange={(e) => setCustomQuery(e.target.value)} className="px-4 py-2 border border-orange-200 rounded-lg focus:ring-2 focus:ring-orange-400 outline-none text-sm w-full" />
      ...
      <div className="p-8 max-w-6xl mx-auto w-full">
        <h2 className="font-serif text-3xl font-bold text-orange-900 mb-8">{title}</h2>
                  ...
                  {day.meals.map((meal, mIdx) => (
                    <div key={mIdx} onClick={() => setSelectedMeal(meal)} className="bg-white border border-orange-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all cursor-pointer group">
                       ...
                       <div className="p-4">
                          ...
                          <h4 className="font-bold text-gray-900 mb-1 group-hover:text-orange-600 transition-colors">{meal.dish_name}</h4>
                          <p className="text-[10px] text-gray-500 line-clamp-2">{meal.benefit}</p>
                       </div>
                    </div>
                  ))}
                </div>
